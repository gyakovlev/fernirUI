...
local function makeButton(id, text, x, y)
	local result = CreateFrame("Button", id, InboxFrame)
	local label = result:CreateFontString(nil,"OVERLAY","GameFontNormal")
	label:SetText(text)
	result:SetWidth(label:GetWidth() + 10)
	result:SetHeight(label:GetHeight()+10)
	result:SetFontString(label)
	result:SetPoint("CENTER", InboxFrame, "TOP", x, y)
	stylef(result)
	return result
end

button = makeButton("OpenAllButton", "Take All", -50, -410)
button:SetScript("OnClick", openAll)
button:SetScript("OnEvent", onEvent)
button2 = makeButton("OpenAllButton2", "Take Cash", 20, -410)
button2:SetScript("OnClick", openAllCash)
